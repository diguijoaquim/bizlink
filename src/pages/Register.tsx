...
import { registerUser, loginWithPassword, saveAuthToken, loginWithGoogle } from "@/lib/api";
...
export default function Register() {
  ...
  async function handleGoogleRegister() {
    ...
    try {
        ...
        callback: async (response: any) => {
          try {
            if (response.access_token) {
              ...
              if (userInfo.email) {
                // Create a mock credential for our loginWithGoogle function
                ...
                saveAuthToken(res.access_token);
                try { toast({ title: "Conta criada com Google" }); } catch {}
                navigate("/profile");
              } else {
            ...
            setError(err?.message || "Falha ao registar com Google");
          } finally {
            setGoogleLoading(false);
          }
        },
        error_callback: (error: any) => {
          setError("Erro na autenticação Google: " + (error.message || "Desconhecido"));
          setGoogleLoading(false);
        }
      });
      ...
      setError(err?.message || "Erro ao inicializar Google Sign-In");
      setGoogleLoading(false);
    }
  }
    ...
    <div className="min-h-screen bg-gradient-soft flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="bg-card rounded-2xl p-6 shadow-bizlink-soft">
          ...
          <Button 
            variant="outline" 
            className="w-full h-11 border-border hover:bg-muted mb-6"
          ...
          >
            ...
            {googleLoading ? "Conectando..." : "Registar com Google"}
          </Button>
